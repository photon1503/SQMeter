...
namespace ASCOM.SQMeter.ObservingConditions
{
    ...
    public partial class SetupDialogForm : Form
    {
        ...
        private void CmdOK_Click(object sender, EventArgs e) // OK button event handler
        {
            ...
            // Update the polling interval variable if a valid number has been entered
            if (int.TryParse(txtPollingInterval.Text, out int pollingInterval))
            {
                ObservingConditionsHardware.pollingInterval = pollingInterval;
                tl.LogMessage("Setup OK", $"New configuration values - Polling Interval: {pollingInterval}");
            }
            else
            {
                tl.LogMessage("Setup OK", $"New configuration values - Polling Interval: Invalid value entered, using default value of {ObservingConditionsHardware.pollingInterval}");
            }

            if (int.TryParse(txtAveragePeriod.Text, out int averagePeriod))
            {
                ObservingConditionsHardware.averagePeriod = averagePeriod;
                tl.LogMessage("Setup OK", $"New configuration values - Average Period: {averagePeriod}");
            }
            else
            {
                tl.LogMessage("Setup OK", $"New configuration values - Average Period: Invalid value entered, using default value of {ObservingConditionsHardware.averagePeriod}");
            }
        }
        ...
        private void InitUI()
        {
            ...
            txtPollingInterval.Text = ObservingConditionsHardware.pollingInterval.ToString();
            txtAveragePeriod.Text = ObservingConditionsHardware.averagePeriod.ToString();
